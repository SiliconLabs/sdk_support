...
static void rx_do_dma(uint8_t * rx_buf, uint16_t xlen)
{
     ...
     */
    dummy_data = 0;
    DMADRV_PeripheralMemory(rx_dma_chan, MY_USART_RX_SIGNAL, (void *) rx_buf, (void *) &(MY_USART->RXDATA), true, xlen,
                            dmadrvDataSize1, rx_dma_complete, NULL);

    // Start transmit DMA.
    DMADRV_MemoryPeripheral(tx_dma_chan, MY_USART_TX_SIGNAL, (void *) &(MY_USART->TXDATA), (void *) &(dummy_data), false, xlen,
...
static void tx_do_dma(uint8_t * rx_buf, uint8_t * tx_buf, uint16_t xlen)
{
    ...
    DMADRV_PeripheralMemory(rx_dma_chan, MY_USART_RX_SIGNAL, buf, (void *) &(MY_USART->RXDATA), srcinc, xlen, dmadrvDataSize1,
                            rx_dma_complete, buf);
    // Start transmit DMA.
    DMADRV_MemoryPeripheral(tx_dma_chan, MY_USART_TX_SIGNAL, (void *) &(MY_USART->TXDATA), (void *) tx_buf, true, xlen,
                            dmadrvDataSize1, NULL, NULL);
#endif /* USE_LDMA */
}
/*
