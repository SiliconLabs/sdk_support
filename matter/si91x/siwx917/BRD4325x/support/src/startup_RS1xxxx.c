...
#define EXT_IRQ_COUNT                                                          \
  98 /**< Number of External (NVIC) interrupts */ // senthil copied from
                                                  // "efr32mg12p432f1024gl125.h"
...
extern void _start(void)
    __attribute__((noreturn)); /* Pre Main (C library entry point) */
#else
...
/**
 *@brief The minimal vector table for a Cortex M4.  Note that the proper
 *constructs must be placed on this to ensure that it ends up at physical
 *address 0x00000000.
 */
...
const tVectorEntry __VECTOR_TABLE[TOTAL_INTERRUPTS]
    __attribute__((aligned(512))) __VECTOR_TABLE_ATTRIBUTE = {
        ...
        {(void *)0x300001},   /* Reset Handler : IN CCP Platform system start
                                             from bootloader which inturn call
                                 reset handler */
