...
#define PSA_KEY_TYPE_ECC_GET_FAMILY(type)                                      \
  ((psa_ecc_family_t)(                                                         \
      PSA_KEY_TYPE_IS_ECC(type) ? ((type)&PSA_KEY_TYPE_ECC_CURVE_MASK) : 0))
...
#define PSA_KEY_TYPE_DH_GET_FAMILY(type)                                       \
  ((psa_dh_family_t)(                                                          \
      PSA_KEY_TYPE_IS_DH(type) ? ((type)&PSA_KEY_TYPE_DH_GROUP_MASK) : 0))
...
#define PSA_ALG_IS_HASH_AND_SIGN(alg)                                          \
  (PSA_ALG_IS_SIGN_HASH(alg) && ((alg)&PSA_ALG_HASH_MASK) != 0)
...
mbedtls_svc_key_id_make(mbedtls_key_owner_id_t owner_id, psa_key_id_t key_id) {
  return ((mbedtls_svc_key_id_t){.key_id = key_id, .owner = owner_id});
}
