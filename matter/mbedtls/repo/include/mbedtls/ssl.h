 ...
 * received. \returns        \c 0 if the connection has been closed. \returns If
 * performing non-blocking I/O, \c MBEDTLS_ERR_SSL_WANT_READ must be returned
 * when the operation would block. \returns        Another negative error code
...
MBEDTLS_DEPRECATED extern int (*mbedtls_ssl_hw_record_activate)(
    mbedtls_ssl_context *ssl, int direction);
 ...
 */
int mbedtls_ssl_set_cid(mbedtls_ssl_context *ssl, int enable,
                        unsigned char const *own_cid, size_t own_cid_len);
 ...
 * \return            A negative error code on failure.
 */
int mbedtls_ssl_get_peer_cid(
    mbedtls_ssl_context *ssl, int *enabled,
 ...
 */
int mbedtls_ssl_check_record(mbedtls_ssl_context const *ssl, unsigned char *buf,
                             size_t buflen);
...
void mbedtls_ssl_conf_export_keys_ext_cb(
    mbedtls_ssl_config *conf, mbedtls_ssl_export_keys_ext_t *f_export_keys_ext,
    void *p_export_keys);
#endif /* MBEDTLS_SSL_EXPORT_KEYS */
 ...
 * value is #MBEDTLS_SSL_DTLS_SRTP_MKI_UNSUPPORTED.
 *
 ...
 *
 * \param conf              SSL configuration
 * \param profiles          Pointer to a List of MBEDTLS_TLS_SRTP_UNSET
...
int mbedtls_ssl_conf_dtls_srtp_protection_profiles(
    mbedtls_ssl_config *conf, const mbedtls_ssl_srtp_profile *profiles);
 ...
 *                 HelloRequest is retransmitted every time mbedtls_ssl_read()
 ...
 * \return         Current maximum fragment length for the output buffer.
 */
MBEDTLS_DEPRECATED size_t
mbedtls_ssl_get_max_frag_len(const mbedtls_ssl_context *ssl);
#endif /* MBEDTLS_DEPRECATED_REMOVED */
#endif /* MBEDTLS_SSL_MAX_FRAGMENT_LENGTH */
 ...
 */
int mbedtls_ssl_context_save(mbedtls_ssl_context *ssl, unsigned char *buf,
 ...
 * \return         #MBEDTLS_ERR_SSL_BAD_INPUT_DATA if input data is invalid.
 */
int mbedtls_ssl_context_load(mbedtls_ssl_context *ssl, const unsigned char *buf,
                             size_t len);
