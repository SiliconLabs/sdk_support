 ...
 * pointers(`p_data`, `p_data_length`) is invalid. For example is `NULL` or
 * references memory the caller cannot access. In addition, this can also happen
 * if an invalid offset was provided.
 */
 ...
 * because one of the provided pointers(`p_info`) is invalid, for example is
