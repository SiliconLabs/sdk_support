...
psa_status_t mbedtls_test_record_status(psa_status_t status, const char *func,
...
#define MBEDTLS_TEST_PSA_SKIP_IF_ALT_AES_192(key_type, key_bits)               \
  do {                                                                         \
    if ((MBEDTLS_TEST_HAVE_ALT_AES) && ((key_type) == PSA_KEY_TYPE_AES) &&     \
        (key_bits == 192)) {                                                   \
...
#define MBEDTLS_TEST_PSA_SKIP_IF_ALT_GCM_NOT_12BYTES_NONCE(alg, nonce_length)  \
  do {                                                                         \
    if ((MBEDTLS_TEST_HAVE_ALT_GCM) &&                                         \
        ...
        ((nonce_length) != 12)) {                                              \
      mbedtls_test_skip("GCM with non-12-byte IV is not supported", __LINE__,  \
                        __FILE__);                                             \
      goto exit;                                                               \
    }                                                                          \
  } while (0)
