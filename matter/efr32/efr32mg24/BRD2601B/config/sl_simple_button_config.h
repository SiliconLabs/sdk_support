...
// <i> Default: 5
// <i> Defines the number of calls to sl_simple_button_poll_step wherein the
// button <i> state must remain the same before the button state is considered
// debounced
...
// <i> SL_SIMPLE_BUTTON_GPIO_MODE == gpioModeInputPull, DOUT determines pull
// direction <i> SL_SIMPLE_BUTTON_GPIO_MODE == gpioModeInputPullFilter, DOUT
...
// <q SL_SIMPLE_BUTTON_ALLOW_LED_CONFLICT> Allow the app to manage Buttons and
// LEDs on the same pin <i> 0  Error if Buttons and LEDs are on the same pin <i>
// 1  Do not error if Buttons and LEDs are on the same pin <i> Default: 0
