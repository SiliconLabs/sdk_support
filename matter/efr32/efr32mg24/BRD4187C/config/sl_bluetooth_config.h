...
#define SL_BT_CONFIG_MAX_CONNECTIONS_SUM                                       \
  (SL_BT_CONFIG_MAX_CONNECTIONS + SL_BT_COMPONENT_CONNECTIONS)
...
#define SL_BT_CONFIG_MAX_ADVERTISERS                                           \
...
// <i> Default: 4
// <i> Define the number of software timers the application needs.  Each timer
// needs resources from the stack to be implemented. Increasing amount of soft
// timers may cause degraded performance in some use cases.
#define SL_BT_CONFIG_MAX_SOFTWARE_TIMERS (4)

// <o SL_BT_CONFIG_BUFFER_SIZE> Buffer memory size for Bluetooth stack
// <i> Default: 3150
// <i> Define buffer memory size for running Bluetooth stack and buffering data
// over Bluetooth connections, <i> advertising and scanning. The default value
// is an estimation for achieving adequate throughput <i> and supporting
// multiple simultaneous connections. Consider increasing this value for <i>
// higher data throughput over connections, advertising or scanning long
// advertisement data.
...
// <i> Default: -30 (-3 dBm)
// <i> Configure the minimum radiated TX power level for Bluetooth connections
// and DTM testing. <i> This configuration is used for power control on
// Bluetooth connections <i> if the LE Power Control feature is enabled. <i>
// When this configuration is passed into stack initialization, the stack <i>
// will select the closest value that the device supports. <i> API
// sl_bt_system_get_tx_power_setting() can be used to query the selected value.
...
// <i> API sl_bt_system_get_tx_power_setting() can be used to query the selected
// value.
...
// <i> Default: 0
// <i> The Bluetooth stack takes TX RF path gain into account when adjusting
// transmitter <i> output power. Power radiated from the antenna then matches
// the application request. <i> A negative value indicates some power loss in
// the path. For example, <i> with radiated TX power set to +10 dBm and this
// configuration to -10 <i> (i.e., 1 dBm loss), the transmitter output power
// will be set to +11 dBm.
#define SL_BT_CONFIG_RF_PATH_GAIN_TX (0)

// <o SL_BT_CONFIG_RF_PATH_GAIN_RX> RF RX path gain in 0.1dBm unit
// <i> Default: 0
// <i> RX RF path gain is used to compensate the RSSI reports from the Bluetooth
// Stack.
...
#define SL_BT_CONFIG_DEFAULT                                                   \
  {                                                                            \
    ...
    .stack_schedule_callback = SL_BT_CONFIG_STACK_CALLBACK, .gattdb = &gattdb, \
