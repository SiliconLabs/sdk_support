    ...
    psa_key_handle_t handle )
{
     ...
     * handle the given lifetime us registered.  */
    if( psa_key_lifetime_is_external( slot->attr.lifetime ) ) {
         ...
         * Since only salt is allowed to be input before the key, we can be sure
         * that no info array has been allocated, and we don't need to free
         * anything. */
        const uint8_t * salt_ptr = operation->ctx.hkdf.salt;
        size_t salt_length = operation->ctx.hkdf.salt_length;
        memset( &operation->ctx.hkdf, 0, sizeof( operation->ctx ) );
        operation->ctx.opaque_kdf.salt = (uint8_t *)salt_ptr;
        ...
        case PSA_ALG_ECDH:
            {
                if( ! PSA_KEY_TYPE_IS_ECC_KEY_PAIR( private_key->attr.type ) )
                    return( PSA_ERROR_INVALID_ARGUMENT );
                mbedtls_ecp_keypair *ecp = NULL;
                status = psa_load_ecp_representation( private_key->attr.type,
