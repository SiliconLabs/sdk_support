...
psa_status_t psa_destroy_key( psa_key_handle_t handle )
{
    ...
    psa_status_t overall_status = PSA_SUCCESS; /* stores the first failure message if any */
    ...
    if( PSA_KEY_LIFETIME_GET_PERSISTENCE( slot->attr.lifetime ) != PSA_KEY_PERSISTENCE_VOLATILE )
    {
                                             ...
                                             int export_public_key )
{
    ...
    *data_length = 0;
    ...
    {
        status = psa_driver_wrapper_export_key( slot,
                                                data,
                                                data_size,
                                                data_length );
    }
                           ...
                           psa_key_handle_t *target_handle )
{
    ...
    status = psa_driver_wrapper_copy_key( specified_attributes,
                                          source_slot,
                                          target_slot );
                                   ...
                                   int is_sign )
{
    ...
    if( PSA_ALG_IS_HMAC( full_length_alg ) )
    {
        ...
        mac_size = PSA_HASH_SIZE( hash_alg );
    }
    else
        mac_size = 16;
                               ...
                               size_t *ciphertext_length )
{
    ...
    *ciphertext_length = 0;
    ...
    status = psa_get_key_from_slot( handle, &operation.slot, PSA_KEY_USAGE_ENCRYPT, alg );
    if( status != PSA_SUCCESS )
        return( status );
                               ...
                               size_t *plaintext_length )
{
    ...
    *plaintext_length = 0;
    ...
    status = psa_get_key_from_slot( handle, &operation.slot, PSA_KEY_USAGE_DECRYPT, alg );
    if( status != PSA_SUCCESS )
        return( status );
...
{
    psa_status_t status = PSA_SUCCESS;
    unsigned int mbedtls_in_use = operation->mbedtls_in_use;
    ...
    if( operation->mbedtls_in_use == 0 )
    {
        status = psa_driver_wrapper_key_derivation_abort( &operation->ctx.driver );
        if ( status == PSA_SUCCESS )
            goto exit;
        else
            return( PSA_ERROR_BAD_STATE );
    ...
    if( PSA_ALG_IS_HKDF( kdf_alg ) )
    {
        ...
        mbedtls_free( operation->ctx.hkdf.salt );
...
exit:
    ...
    /* Preserve mbedtls_in_use state */
    operation->mbedtls_in_use = mbedtls_in_use;
    return( status );
}
                                         ...
                                         size_t capacity )
{
    ...
    if ( operation->mbedtls_in_use == 0 )
    {
        status = psa_driver_wrapper_key_derivation_set_capacity( &operation->ctx.driver,
                                                                 capacity );
        if ( status != PSA_ERROR_NOT_SUPPORTED )
        {
            if ( status == PSA_SUCCESS )
    ...
    size_t output_length )
{
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PRF) || \
    defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS) || \
    defined(MBEDTLS_PSA_BUILTIN_ALG_HKDF)
    psa_algorithm_t kdf_alg = psa_key_derivation_get_kdf_alg( operation );
#endif
    ...
    if( operation->mbedtls_in_use == 0 )
    {
        status = psa_driver_wrapper_key_derivation_output_bytes( &operation->ctx.driver,
                                                                 output,
                                                                 output_length );
        if( status != PSA_SUCCESS )
            psa_driver_wrapper_key_derivation_abort( &operation->ctx.driver );
        return status;
    }
...
exit:
    if( status != PSA_SUCCESS )
    {
        /* Preserve the algorithm and mbedtls_in_use and storing_references upon
         * errors, but clear all sensitive state.
        ...
        unsigned int mbedtls_in_use = operation->mbedtls_in_use;
        unsigned int storing_references = operation->storing_references;
        psa_key_derivation_abort( operation );
        operation->alg = alg;
        operation->mbedtls_in_use = mbedtls_in_use;
        operation->storing_references = storing_references;
                                       ...
                                       psa_key_handle_t *handle )
{
    ...
    {
        /* Do not allow output of an opaque key unless one has
         * been input to the KDF. This includes the case where
         * an opaque secret key is input to
         * psa_key_derivation_key_agreement(), since the
         * intermediate shared secret result was transparent. */
        if( psa_key_lifetime_is_external(slot->attr.lifetime) )
            return PSA_ERROR_NOT_PERMITTED;
    }
    ...
    if( operation->mbedtls_in_use == 0 )
    {
        status = psa_driver_wrapper_key_derivation_output_key( &operation->ctx.driver,
                                                               attributes,
                                                               slot );
                                       ...
                                       psa_algorithm_t alg )
{
    ...
    operation->mbedtls_in_use = 0;
    status = psa_driver_wrapper_key_derivation_setup( &operation->ctx.driver,
                                                      alg );
    if( status != PSA_ERROR_NOT_SUPPORTED )
        goto exit;
                                    ...
                                    size_t data_length )
{
    ...
    switch( step )
    {
        case PSA_KEY_DERIVATION_INPUT_SALT:
            ...
            hkdf->salt = mbedtls_calloc( 1, data_length );
            if ( hkdf->salt == NULL )
                return( PSA_ERROR_INSUFFICIENT_MEMORY );
            memcpy( hkdf->salt, data, data_length );
    ...
    size_t data_length )
{
    ...
    /* The storing_references bit has precedence over mbedtls_in_use,
     * since it will only be triggered in the event of an opaque key
     * being input */
    if( operation->storing_references == 1 )
    {
        if( !PSA_ALG_IS_HKDF( operation->alg ) )
        ...
        switch (step)
        {
            case PSA_KEY_DERIVATION_INPUT_INFO:
                if( !(operation->can_output_key == 1) )
                    return( PSA_ERROR_INVALID_ARGUMENT );
                if( operation->ctx.opaque_kdf.info_set == 1 )
                    return( PSA_ERROR_BAD_STATE );
                if( data_length != 0 )
                {
                ...
                operation->ctx.opaque_kdf.info_set = 1;
                break;
            default:
                return( PSA_ERROR_INVALID_ARGUMENT );
        }
        /* Return to avoid the internal computation */
        return( PSA_SUCCESS );
    }

    if( operation->mbedtls_in_use == 0 )
    {
        status = psa_driver_wrapper_key_derivation_input_bytes( &operation->ctx.driver,
                                                                operation->alg,
                                                                data,
                                                                data_length );
        if( status != PSA_SUCCESS )
    ...
    psa_key_handle_t handle )
{
    ...
    if( operation->mbedtls_in_use == 0 )
    {
        status = psa_driver_wrapper_key_derivation_input_key( &operation->ctx.driver,
                                                              slot,
                                                              operation->alg );
        if( status != PSA_SUCCESS )
            psa_driver_wrapper_key_derivation_abort( &operation->ctx.driver );
     ...
     * Ideally, we should also be able to check if an opaque driver that could
     * handle the given lifetime is registered.  */
    if( psa_key_lifetime_is_external( slot->attr.lifetime ) ) {
        ...
        uint8_t * salt_ptr = operation->ctx.hkdf.salt;
        size_t salt_length = operation->ctx.hkdf.salt_length;
        uint8_t * info_ptr = operation->ctx.hkdf.info;
        size_t info_length = operation->ctx.hkdf.info_length;
        uint8_t info_set = operation->ctx.hkdf.info_set;
        memset( &operation->ctx.hkdf, 0, sizeof( operation->ctx ) );
        operation->ctx.opaque_kdf.salt = salt_ptr;
        operation->ctx.opaque_kdf.salt_length = salt_length;
        if( info_set == 1 )
        {
            operation->ctx.opaque_kdf.info = info_ptr;
            operation->ctx.opaque_kdf.info_length = info_length;
            operation->ctx.opaque_kdf.info_set = 1;
        }
        ...
        /* Unset the mbedtls_in_use bit, since that is not true anymore */
        operation->mbedtls_in_use = 0;
                                                ...
                                                size_t peer_key_length )
{
    ...
    if ( operation->mbedtls_in_use == 0 )
    {
        status = psa_key_derivation_check_input_type( step, PSA_KEY_TYPE_DERIVE );
        if( status != PSA_SUCCESS )
            goto exit;
                               ...
                               psa_key_handle_t *handle )
{
    ...
    if ( psa_key_lifetime_is_external( slot->attr.lifetime ) )
    {
        status = psa_driver_wrapper_allocate_key( attributes );
        if( status != PSA_SUCCESS )
            goto exit;
    }
