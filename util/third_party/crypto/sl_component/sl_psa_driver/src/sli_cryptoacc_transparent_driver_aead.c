                                                    ...
                                                    size_t *ciphertext_length)
{
  ...
  status = check_aead_parameters(alg, nonce_length, key_bits);
  ...
  switch (PSA_ALG_AEAD_WITH_TAG_LENGTH(alg, 0)) {
    ...
    case PSA_ALG_AEAD_WITH_TAG_LENGTH(PSA_ALG_GCM, 0):
      ...
      // Libcryptosoc limits AD to 2^64 bits, so 2^61 bytes.
                                                    ...
                                                    size_t *plaintext_length)
{
  ...
  switch (PSA_ALG_AEAD_WITH_TAG_LENGTH(alg, 0)) {
...

    {
      ...
      } else if (sx_ret != CRYPTOLIB_SUCCESS
                 || status != PSA_SUCCESS) {
        return PSA_ERROR_HARDWARE_FAILURE;
      }
    ...
    case PSA_ALG_AEAD_WITH_TAG_LENGTH(PSA_ALG_GCM, 0):
      ...
      sx_ret = sx_aes_gcm_decrypt_verify((const block_t *)&key,
                                         (const block_t *)&data_in,
                                         &data_out,
                                         (const block_t *)&nonce_internal,
                                         &tag_block,
