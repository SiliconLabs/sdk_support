def validate(project):
    ...
    for inst in project.component('spidrv_usart').instances():
        opt = project.config('SL_SPIDRV_USART_{}_CS_CONTROL'.format(inst.upper()))
        cs = project.config('SL_SPIDRV_USART_{}_CS_PORT'.format(inst.upper()))
        ...
        if (opt is not None) and (cs is not None):
            if opt.value() == 'spidrvCsControlAuto' and not cs:
                project.error(
                    ...
                    project.target_for_defines(['SL_SPIDRV_USART_{}_CS_CONTROL'.format(inst.upper())]),
