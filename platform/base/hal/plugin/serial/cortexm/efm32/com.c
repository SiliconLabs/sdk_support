...
#define COM_USART_ENABLE
#endif

#if (defined(COM_LEUART0_ENABLE) \
  || defined(COM_LEUART1_ENABLE))
#define COM_LEUART_ENABLE
#endif

// Set COM_UART_ENABLE if at least one UART is enabled
#if (defined(COM_USART_ENABLE) \
  || defined(COM_LEUART_ENABLE))
  ...
  for (uint8_t i = 0; i < comHandleIndexSize; i++) {
    ...
    if (comhandle->uarthandle == handle) {
      ...
      while (UARTDRV_GetReceiveDepth(handle) < 3) {
        // Do not set up the next buffer if the threshold is already reached
        if (comhandle->rxQueue->used > (comhandle->rxsize - 2 * comhandle->rxStop)) {
...
static void COM_RX_IRQHandler(COM_Handle_t comhandle, uint8_t byte)
{
  ...
  if (byte == UARTDRV_FC_SW_XON) {
    ...
    // store byte in RX FIFO only if there is space available
...
// leuart: return true if txc is true or hastransmitted is false
bool COM_InternalTxIsIdle(COM_Port_t port)
{
  ...
  if (checkValidUartPort(port)) {
    COM_Handle_t comhandle = getComHandleFromPort(port);
    if ((UARTDRV_GetPeripheralStatus(comhandle->uarthandle) & UARTDRV_STATUS_TXIDLE)
        && (comhandle->txQueue->used == 0)
        && (UARTDRV_GetTransmitDepth(comhandle->uarthandle) == 0)) {
      #ifdef COM_USART_ENABLE
      if (comhandle->uarthandle->type == uartdrvUartTypeUart) {
        // in USART cases check the TXBUFCNT register as well as TXIDLE: EMHAL-2028
        return ((UARTDRV_GetPeripheralStatus(comhandle->uarthandle) & _USART_STATUS_TXBUFCNT_MASK) == 0);
      }
      #endif // COM_USART_ENABLE

      #ifdef COM_LEUART_ENABLE
      if (comhandle->uarthandle->type == uartdrvUartTypeLeuart) {
        // in LEUART cases check if TXC is flagged, as long as we have already transmitted
        return ((UARTDRV_GetPeripheralStatus(comhandle->uarthandle) & UARTDRV_STATUS_TXC)
                || !(comhandle->uarthandle->hasTransmitted));
      }
    #endif // COM_LEUART_ENABLE
    }
  }
