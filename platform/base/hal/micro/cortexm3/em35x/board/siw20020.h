 ...
 * above.
 */
#define BUTTON_RECOVERY_SET()     (GPIO->P[0].SET = GPIO_P_SET_Px5)

/**
 * @brief The GPIO input method to sense whether the button is pressed.
 * Must match the port-pin assignment above.
 */
#define BUTTON_RECOVERY_PRESSED() ((GPIO->P[0].IN & GPIO_P_IN_Px5) ? false : true)
...
// Configure PB7 for EEPROM shutdown
  #define PWRUP_CFG_EEPROM  GPIO_P_CFGz_Pxy_OUT
  #define PWRUP_OUT_EEPROM  0
  #define PWRDN_CFG_EEPROM  GPIO_P_CFGz_Pxy_OUT
  #define PWRDN_OUT_EEPROM  0
#else//!EEPROM_USES_SHUTDOWN_CONTROL
// Configure PB7 for buzzer
  #define PWRUP_CFG_EEPROM  GPIO_P_CFGz_Pxy_OUT_ALT
  #define PWRUP_OUT_EEPROM  0
  #define PWRDN_CFG_EEPROM  GPIO_P_CFGz_Pxy_IN_PUD
  #define PWRDN_OUT_EEPROM  GPIO_P_OUT_Pxy_PULLDOWN
...
// Configure PA4 and PA5 for LED, GPIO
  #define PWRUP_CFG_PTI_EN    GPIO_P_CFGz_Pxy_OUT
  #define PWRUP_OUT_PTI_EN    1
  #define PWRDN_CFG_PTI_EN    GPIO_P_CFGz_Pxy_OUT
...
// Configure PC7 for OSC32K digital signal
  #define CFG_OSC32K         GPIO_P_CFGz_Pxy_IN
#else // Generic GPIO usage
// Configure PC7 for TEMP_EN
  #define CFG_OSC32K         GPIO_P_CFGz_Pxy_OUT
#endif
...
#define DEFINE_POWERUP_GPIO_CFG_VARIABLES()                     \
  ...
  uint16_t gpioCfgPowerUp[6] = {                                \
    ...
    ((PWRUP_CFG_PTI_EN   << _GPIO_P_CFGH_Px4_SHIFT)             \
     ...
     | (GPIO_P_CFGz_Pxy_OUT        << _GPIO_P_CFGH_Px6_SHIFT)   \
     | (GPIO_P_CFGz_Pxy_IN         << _GPIO_P_CFGH_Px7_SHIFT)), \
...
#define DEFINE_POWERUP_GPIO_OUTPUT_DATA_VARIABLES()         \
  uint8_t gpioOutPowerUp[3] = {                             \
    ...
    ((1                  << _GPIO_P_OUT_Px0_SHIFT)          \
     ...
     | (GPIO_P_OUT_Pxy_PULLUP     << _GPIO_P_OUT_Px2_SHIFT) \
     | (GPIO_P_OUT_Pxy_PULLUP     << _GPIO_P_OUT_Px3_SHIFT) \
     | (0                  << _GPIO_P_OUT_Px4_SHIFT)        \
     | (1                  << _GPIO_P_OUT_Px5_SHIFT)        \
     | (GPIO_P_OUT_Pxy_PULLUP     << _GPIO_P_OUT_Px6_SHIFT) \
     | (PWRUP_OUT_EEPROM   << _GPIO_P_OUT_Px7_SHIFT)),      \
...
#define DEFINE_POWERDOWN_GPIO_CFG_VARIABLES()                   \
  uint16_t gpioCfgPowerDown[6] = {                              \
    ...
    ((GPIO_P_CFGz_Pxy_OUT        << _GPIO_P_CFGL_Px0_SHIFT)     \
     ...
     | (GPIO_P_CFGz_Pxy_IN_PUD     << _GPIO_P_CFGL_Px2_SHIFT)   \
     | (GPIO_P_CFGz_Pxy_IN_PUD     << _GPIO_P_CFGL_Px3_SHIFT)), \
    ((GPIO_P_CFGz_Pxy_OUT        << _GPIO_P_CFGH_Px4_SHIFT)     \
     | (GPIO_P_CFGz_Pxy_OUT        << _GPIO_P_CFGH_Px5_SHIFT)   \
     | (GPIO_P_CFGz_Pxy_IN_PUD     << _GPIO_P_CFGH_Px6_SHIFT)   \
     | (PWRDN_CFG_EEPROM   << _GPIO_P_CFGH_Px7_SHIFT)),         \
...
#define DEFINE_POWERDOWN_GPIO_OUTPUT_DATA_VARIABLES()         \
  uint8_t gpioOutPowerDown[3] = {                             \
    ((PWRDN_OUT_USBDM    << _GPIO_P_OUT_Px0_SHIFT)            \
     ...
     | (GPIO_P_OUT_Pxy_PULLUP     << _GPIO_P_OUT_Px7_SHIFT)), \
    ((0                  << _GPIO_P_OUT_Px0_SHIFT)            \
     | (1                  << _GPIO_P_OUT_Px1_SHIFT)          \
     | (GPIO_P_OUT_Pxy_PULLUP     << _GPIO_P_OUT_Px2_SHIFT)   \
     | (GPIO_P_OUT_Pxy_PULLDOWN   << _GPIO_P_OUT_Px3_SHIFT)   \
     | (PWRDN_OUT_SC1_nRTS << _GPIO_P_OUT_Px4_SHIFT)          \
     | (0                  << _GPIO_P_OUT_Px5_SHIFT)          \
     | (GPIO_P_OUT_Pxy_PULLUP     << _GPIO_P_OUT_Px6_SHIFT)   \
     | (PWRDN_OUT_EEPROM   << _GPIO_P_OUT_Px7_SHIFT)),        \
...
#define SET_SUSPEND_GPIO_OUTPUT_DATA_REGISTERS() do {                                           \
    ...
    enum { PA_USB_MASK = (GPIO_P_OUT_Px0 | GPIO_P_OUT_Px1 | GPIO_P_OUT_Px2 | GPIO_P_OUT_Px3) }; \
    GPIO->P[0].OUT = (GPIO->P[0].OUT & PA_USB_MASK)                                             \
