...
{
  if (!RAIL_IsPaAutoModeEnabled(railHandle)) {
...
void configRfSenseSelectiveOokWakeupPhy(int argc, char **argv)
{
  ...
  responsePrint(argv[0], "RFSense Wakeup PHY:%s", status ? "Disabled" : "Enabled");
}
